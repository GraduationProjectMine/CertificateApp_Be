...
export class CertificateController {
  ...
  async createDraft(@Req() req: AuthenticatedRequest, @Body() dto: CreateCertificateDto) {
    const user = req.user;
  ...
  async findAllOnline(@Req() req: AuthenticatedRequest) {
    const user = req.user;
    if (user.role === 'student') {
  ...
  async batchApprove(@Req() req: AuthenticatedRequest, @Body() dto: BatchApproveDto) {
    const user = req.user;
    if (user.role !== 'issuer') {
      throw new ForbiddenException(
  ...
  async retryRevoke(@Req() req: AuthenticatedRequest, @Param('id') id: string) {
    const user = req.user;
    if (user.role !== 'issuer') {
      throw new ForbiddenException(
