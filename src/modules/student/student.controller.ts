...
export class StudentController {
  ...
  async importHistory(@Req() req: AuthenticatedRequest) {
    const user = req.user;
    if (user.role !== 'issuer' && user.role !== 'staff') {
      throw new ForbiddenException(
  ...
  async updateProfile(@Req() req: AuthenticatedRequest, @Body() dto: UpdateProfileDto) {
    ...
    const student = await this.studentService.updateProfile(user.id, dto);
  ...
  async create(@Req() req: AuthenticatedRequest, @Body() dto: CreateStudentDto) {
    const user = req.user;
    if (user.role !== 'issuer' && user.role !== 'staff') {
      throw new ForbiddenException(
  ...
  async import(@Req() req: AuthenticatedRequest, @UploadedFile() file: Express.Multer.File) {
    const user = req.user;
    if (user.role !== 'issuer' && user.role !== 'staff') {
      throw new ForbiddenException(
